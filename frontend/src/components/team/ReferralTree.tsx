  ...
  if (!address) {
    return (
      <div className="bg-white border border-slate-200 rounded-xl p-6">
        <h3 className="text-lg font-semibold text-slate-900 mb-4">Referral Team</h3>
        <p className="text-slate-500">Connect your wallet to view your team</p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-6">
          ...
          <div className="flex items-center gap-2">
            <input
              readOnly
              value={referralLink}
              className="flex-1 bg-slate-100 border border-slate-300 rounded-lg px-4 py-2 text-sm text-slate-600 font-mono"
            />
            <button
              onClick={copyLink}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white border border-slate-200 rounded-xl p-4">
          ...
          <p className="text-2xl font-bold text-slate-900">{downlineCount?.toString() || "0"}</p>
        </div>
        <div className="bg-white border border-slate-200 rounded-xl p-4">
          <p className="text-sm text-slate-500">Your Referrer</p>
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-4">
          <p className="text-sm text-slate-500">Levels Unlocked</p>
        ...
        {directDownlines && directDownlines.length > 0 ? (
          <div className="space-y-2">
            {directDownlines.map((downline: string, i: number) => (
              <div key={downline} className="flex items-center justify-between bg-slate-100 rounded-lg p-3">
                <div className="flex items-center gap-3">
                  <span className="text-xs bg-blue-600 text-white w-6 h-6 rounded-full flex items-center justify-center">
                    {i + 1}
                  </span>
                  <span className="font-mono text-sm text-slate-600">{shortenAddress(downline)}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-xs">
          {[
            ...
            <div key={item.level} className={`rounded p-2 border ${item.unlocked ? "bg-green-50 border-green-200" : "bg-slate-100 border-slate-300"}`}>
              <p className={`font-medium ${item.unlocked ? "text-green-600" : "text-slate-900"}`}>{item.level}: {item.rate}</p>
              <p className={item.unlocked ? "text-green-600" : "text-slate-400"}>
