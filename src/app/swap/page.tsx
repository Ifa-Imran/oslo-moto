...
export default function SwapPage() {
  ...
  const { data: busdBal } = useBalance({
    address,
    token: CONTRACTS.busd as `0x${string}`,
  });
  ...
  const [swapDirection, setSwapDirection] = useState<"BUSD_TO_OSLO" | "OSLO_TO_BUS">("BUSD_TO_OSLO");
  const [showSlippageSettings, setShowSlippageSettings] = useState(false);

  const estimatedOutput = getEstimatedOutput(swapInput, swapDirection === "BUSD_TO_OSLO");
  
  const handleMax = () => {
    if (swapDirection === "BUSD_TO_OSLO" && busdBal) {
      setSwapInput((Number(busdBal.formatted) - 0.01).toString()); // Leave some for gas
    } else if (swapDirection === "OSLO_TO_BUS" && osloBal) {
  ...
  const handleSwap = async () => {
    ...
    if (swapDirection === "BUSD_TO_OSLO") {
      await handleSwapBUSDForOSLO(swapInput);
    } else {
      await handleSwapOSLOForBUSD(swapInput);
    }
  };

  const switchDirection = () => {
    setSwapDirection(swapDirection === "BUSD_TO_OSLO" ? "OSLO_TO_BUS" : "BUSD_TO_OSLO");
    setSwapInput("");
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-950 via-purple-950 to-indigo-950 py-12 px-4">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-400">Swap BUSD ↔ OSLO with protocol-controlled liquidity</p>
        </div>
        ...
        <div className="bg-gradient-to-r from-blue-900/40 to-purple-900/40 backdrop-blur-xl border border-blue-500/30 rounded-2xl p-6 mb-6">
          <div className="grid grid-cols-3 gap-4 text-center">
            <div>
              ...
              <p className="text-2xl font-bold text-white">{parseFloat(price).toFixed(6)} BUSD</p>
            </div>
            <div>
              <p className="text-sm text-gray-400 mb-1">BUSD Reserve</p>
              <p className="text-xl font-semibold text-blue-300">{parseFloat(busdReserve).toLocaleString(undefined, { maximumFractionDigits: 0 })}</p>
            </div>
            <div>
        ...
        <div className="bg-gradient-to-br from-blue-900/50 to-purple-900/50 backdrop-blur-xl border border-blue-500/40 rounded-3xl p-8 shadow-2xl">
          ...
          <div className="mb-4">
            <div className="flex justify-between items-center mb-2">
              ...
              <div className="flex items-center gap-2">
                <span className="text-sm text-gray-400">
                  Balance: {swapDirection === "BUSD_TO_OSLO" ? busdBal?.formatted || "0" : osloBal?.formatted || "0"}
                </span>
                <button
                  onClick={handleMax}
            ...
            <div className="bg-blue-950/50 border border-blue-500/30 rounded-xl p-4">
              ...
              <div className="flex items-center gap-2 mt-2">
                <div className={`w-8 h-8 rounded-full ${swapDirection === "BUSD_TO_OSLO" ? "bg-green-500" : "bg-blue-500"} flex items-center justify-center text-white font-bold text-sm`}>
                  {swapDirection === "BUSD_TO_OSLO" ? "B" : "O"}
                </div>
                <span className="text-white font-semibold">
                  {swapDirection === "BUSD_TO_OSLO" ? "BUSD" : "OSLO"}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="mb-6">
            ...
            <div className="bg-purple-950/50 border border-purple-500/30 rounded-xl p-4">
              ...
              <div className="flex items-center gap-2 mt-2">
                <div className={`w-8 h-8 rounded-full ${swapDirection === "BUSD_TO_OSLO" ? "bg-blue-500" : "bg-green-500"} flex items-center justify-center text-white font-bold text-sm`}>
                  {swapDirection === "BUSD_TO_OSLO" ? "O" : "B"}
                </div>
                <span className="text-white font-semibold">
                  {swapDirection === "BUSD_TO_OSLO" ? "OSLO" : "BUSD"}
                </span>
              </div>
            </div>
          </div>
        ...
        <div className="mt-8 bg-blue-900/30 backdrop-blur-xl border border-blue-500/30 rounded-2xl p-6">
          ...
          <ul className="space-y-2 text-sm text-gray-300">
            ...
            <li>• <strong>Fair Pricing:</strong> Price = BUSD Reserve / OSLO Reserve</li>
